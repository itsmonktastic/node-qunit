...
print.xunit = function() {
    var currentTest, test;
    var assertionsForTest = _(data.assertions).groupBy('test');

    log('<testsuite>');
    _(data.tests).each(function(test) {
        if (test.failed === 0) {
            log('  <testcase classname="'+test.module+'" name="'+test.name+'" />');
            return;
        }

        var assertions = assertionsForTest[test.name];
        var failed = _(assertions).filter(function (a) { return !a.result; });
        var indent = '  ';
        log(indent+'<testcase classname="'+test.module+'" name="'+test.name+'">');
        indent = '    ';
        _(failed).each(function(assertion) {
            log(indent+'<failure type="AssertionError">'+assertion.message+'</failure>');
        });
        indent = '  ';
        log(indent+'</testcase>');
    });
